...
bool SceneSerializer::deserialize(const std::string& filepath, Context& device, SceneData& outSceneData)
{
...
Material MaterialSerializer::deserialize(const std::string& filepath, class Context& device)
{
	std::ifstream inFile(filepath);
	std::stringstream ss;
	ss << inFile.rdbuf();
